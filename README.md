...
a role, albeit weaker than I initially thought. `HTODemux` currently
does not consider this source of bias and could benefit from doing so.
An easy, perhaps naive, way of implementing this would be to first
compute cell intrinsic factors, correct hash tag counts for these
factors, and then run `HTODemux` as usual.
